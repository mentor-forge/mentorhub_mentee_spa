/**
 * Deployment-boundary coverage for the `/mentee/` nginx + Vite base model.
 *
 ...
 * `/mentee/api/*` reaches mentee_api (including the regex-vs-prefix precedence edge).
 */
describe('Deployment boundary (/mentee/ nginx)', () => {
  ...
  it('redirects / and /mentee to /mentee/', () => {
    cy.request({ url: '/', followRedirect: false }).then((response) => {
      expect(response.status).to.be.oneOf([301, 302])
      expect(response.headers.location).to.equal('/mentee/')
    })

    cy.request({ url: '/mentee', followRedirect: false }).then((response) => {
      expect(response.status).to.be.oneOf([301, 302])
      expect(response.headers.location).to.equal('/mentee/')
    })
  })

  it('serves the Mentee SPA shell at /mentee/ with no-store (not immutable)', () => {
    cy.request('/mentee/').then((response) => {
      expect(response.status).to.equal(200)
      expect(contentType(response.headers)).to.include('text/html')
      expect(String(response.body)).to.include('Mentee')
      expect(String(response.body)).to.include('/mentee/runtime-config.js')
      expect(String(response.body)).to.match(/\/mentee\/assets\/.+\.js/)
  ...
  it('falls back history routes under /mentee/ to the SPA document with no-store', () => {
    cy.request('/mentee/paths/does-not-exist-yet').then((response) => {
      expect(response.status).to.equal(200)
      expect(contentType(response.headers)).to.include('text/html')
      expect(String(response.body)).to.include('/mentee/runtime-config.js')
  ...
  it('serves versioned /mentee/assets with public, immutable caching', () => {
    cy.request('/mentee/').then((shell) => {
      const match = /\/mentee\/assets\/[^"']+\.js/.exec(String(shell.body))
  ...
  it('serves /mentee/runtime-config.js with no-store and the Mentee IdP URI', () => {
    cy.request('/mentee/runtime-config.js').then((prefixed) => {
      ...
      // serve another journey's root-level runtime-config when the browser asks for /mentee/.
  ...
  it('does not let the static-asset regex swallow /mentee/api/*.js', () => {
    // Without `^~` on `/mentee/api/`, nginx would evaluate the `.js` asset regex and miss
    // the API proxy. A proxied miss comes from mentee_api (JSON or its own 404 page),
    // never the Mentee SPA shell or an immutable asset response.
    cy.request({
      url: '/mentee/api/definitely-not-a-static-asset.js',
      failOnStatusCode: false,
    }).then((response) => {
      expect(String(response.body)).not.to.include('/mentee/runtime-config.js')
      expect(String(response.body)).not.to.include('/mentee/assets/')
      expect(String(response.body)).not.to.include('<title>Mentee</title>')
      expect(cacheControl(response.headers)).not.to.include('immutable')
      expect(response.status).to.be.oneOf([401, 403, 404])
    })
  })

  it('returns an API authorization failure (not the SPA) for unauthenticated /mentee/api', () => {
    cy.request({
      url: '/mentee/api/config',
      failOnStatusCode: false,
    }).then((response) => {
  ...
  it('proxies an authenticated Bearer request through /mentee/api to mentee_api', () => {
