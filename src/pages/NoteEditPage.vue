<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">Edit Note</h1>
      </v-col>
    </v-row>
        ...
        <v-card>
          <v-card-text>
            <AutoSaveField
              ...
              automation-id="note-edit-name-input"
            />
            ...
            <AutoSaveField
              ...
              textarea
              :rows="3"
              automation-id="note-edit-description-input"
            />

            <AutoSaveSelect
              :model-value="note.status || 'active'"
              label="Status"
              :items="statusOptions"
              :on-save="(value: string) => updateField('status', value)"
              class="mt-4"
              automation-id="note-edit-status-select"
            />
            ...
            <v-row>
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(note.created.at_time)"
                  label="Created"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="note.created.by_user"
                  label="Created By"
                  readonly
                  variant="outlined"
                  density="compact"
              ...
              <v-col cols="12" md="6">
                <v-text-field
                  :model-value="formatDate(note.saved.at_time)"
                  label="Last Saved"
                  readonly
                  variant="outlined"
                  density="compact"
                />
                <v-text-field
                  :model-value="note.saved.by_user"
                  label="Last Saved By"
                  readonly
                  variant="outlined"
                  density="compact"
            ...
            <v-card-actions class="px-0 mt-4">
              <v-btn 
                ...
                variant="text"
                data-automation-id="note-edit-back-button"
              >
                Back to List
              </v-btn>
            </v-card-actions>
...
/**
 * Note Edit Page - Showcase of spa_utils AutoSave components
 * 
...
import type { NoteUpdate } from '@/api/types'
...
const { mutateAsync: updateNote } = useMutation({
  mutationFn: (data: NoteUpdate) => api.updateNote(noteId.value, data),
  onSuccess: () => {
    queryClient.invalidateQueries({ queryKey: ['note', noteId.value] })
...
async function updateField(field: keyof NoteUpdate, value: string) {
  try {
    await updateNote({ [field]: value })
  } catch (error) {
    throw error
  }
}
