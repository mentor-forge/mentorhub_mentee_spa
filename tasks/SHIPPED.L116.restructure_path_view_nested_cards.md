...
Restructured `PathViewPage` to a single non-collapsible `MhCard` with path name in title, prominent description, two-column `technologies`/`interests` editors, nested collapsed module/topic `MhCard`s, embedded `ResourceViewCard` per resource, and admin-only collapsed Administration sub-card (status + breadcrumbs). Removed list edit controls.

Testing: `npm run test` passed (43 tests); `npm run build` passed; `npm run container` passed; `npm run cypress:run -- --spec cypress/e2e/path.cy.ts` passed (9 tests).
