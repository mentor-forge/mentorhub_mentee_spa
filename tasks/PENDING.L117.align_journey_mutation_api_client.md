...
- Added `JourneyCompleteInput` and four mutation client methods aligned to running OpenAPI (`advance`, `complete`, `promoteJourneyPath`, `promoteJourneyModule` with URL-encoded module names).
- Extended `Journey.client.test.ts` with mutation coverage including URL encoding.
