...
- `src/router/index.test.ts` — `/config` admin resolve + authenticated non-admin Discovery fallback; `/admin` alias
...
**Plan:** Add Vue `path: '/config'` as the admin-gated Settings host (`requiresAuth` + `requiresRole: 'admin'`), reusing the existing `AdminPage.vue` wrapper. Keep `/admin` as a Vue Router **alias** of `/config` (same named record, same component — no second implementation), matching mentor/discovery so bookmarks still resolve. Leave `PageFrame` props, spa_utils pin, Cypress, and journey/path/resource routes unchanged. Update README In-App Route Table to list `/mentee/config` as the Settings host and `/mentee/admin` as the alias. Add colocated `src/router/index.test.ts` for admin resolve + non-admin Discovery `window.location.replace` gate.

**Done:** `/config` is the Settings host; `/admin` is an alias of the same `Admin` record (`AdminPage.vue` unchanged). README lists `/mentee/config` as the admin Settings host (Token / Config Items / Versions / Enumerators; hosting origin, no `:8080` rewrite) and `/mentee/admin` as the alias.

**Tests:**
- `npm run test` — **pass** (51/51; +4 router gate tests)
...
**Follow-ups:** F135 Cypress catalog / `/mentee/config` click-through. Do not commit (orchestrator).
